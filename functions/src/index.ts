...
export const startMatch = functions.https.onRequest((request, response) => {
  corsHandler(request, response, async () => {
    ...
    const cats: Array<Cat> = data.docs.map(
      (doc: { data: () => Cat; id: string }) => {
        return {
          ...doc.data(),
          ref: doc.id
        };
...
export const matchResult = functions.https.onRequest(
  async (request, response) => {
    corsHandler(request, response, async () => {
      const { matchId, winner } = request.body;

      const matchRef = db.collection("matchs").doc(matchId);
      const matchResponse: any = await matchRef.get();
      ...
      const batch = await db
        ...
        .set(cat1Ref, {
          ...cat1,
          elo: calculateNewRating(
            { ...cat1, projectedElo: getExpected(cat1.elo, cat2.elo) },
            cat1.id === winner
          )
        })
        .set(cat2Ref, {
          ...cat2,
          elo: calculateNewRating(
            { ...cat2, projectedElo: getExpected(cat2.elo, cat1.elo) },
            cat2.id === winner
          )
        })
        .set(matchRef, {
...
export const populateCats = functions.https.onRequest(
  async (request, response) => {
